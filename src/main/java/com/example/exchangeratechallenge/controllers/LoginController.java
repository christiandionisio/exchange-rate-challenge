...
import com.example.exchangeratechallenge.security.AuthRequest;
import com.example.exchangeratechallenge.security.AuthResponse;
import com.example.exchangeratechallenge.security.ErrorLogin;
...
public class LoginController {
    ...
    public Mono<ResponseEntity<?>> login(@RequestBody AuthRequest ar){
        return service.findByEmail(ar.getUsername())
                .map((userDetails) -> {
                    ...
                    if(BCrypt.checkpw(ar.getPassword(), userDetails.getPassword())) {
                        ...
                        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new ErrorLogin(
                                "credenciales incorrectas", new Date()));
                    }
